...
const confirmPasswordSchera: ParamSchema = {
  notEmpty: {
...
export const registerValidator = validate(
  checkSchema({
    ...
    confim_password: confirmPasswordSchera,
      ...
      options: async (value: string, { req }) => {
        ...
        try {
          const [decoded_refresh_token, refresh_token] = await Promise.all([
            verifyToken({ token: value, secretOrPublicKey: process.env.JWR_SECRET_REFRESH_TOEKN as string }),
  ...
  email_verify_token: {
    ...
    custom: {
      options: async (value: string, { req }) => {
        ...
        try {
          const decoded_email_verify = await verifyToken({ token: value, secretOrPublicKey: process.env.JWR_SECRET_EMAIL_VERIFY_TOKEN as string })
            ; (req as Request).decoded_email_verify = decoded_email_verify

          console.log("decoded_refresh_token", decoded_email_verify)
        }
        catch (error) {
          throw new ErrorWithStatus({
            message: capitalize((error as JsonWebTokenError).message),
...
export const resetPasswordValidator = validate(checkSchema({
  password: passwordSchema,
  confrim_password: confirmPasswordSchera,
  forgot_password_token: forgotPasswordToken
}, ["body"]))
...
export const changPasswordValidator = validate(
  checkSchema({
      ...
      password: passwordSchema,
      confirm_password: confirmPasswordSchera
  })
)